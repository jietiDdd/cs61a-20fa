...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[i // 2 if i % 2 == 0 else (i + len(deck)) // 2] for i in range(0, len(deck))]
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1):
        ...
        len1 = len(branches1)
        len2 = len(branches2)
        zipped = zip(branches1, branches2)
        return tree(label(t1) + label(t2), [add_trees(b1, b2) for b1, b2 in zipped] + (branches1[len2:] if len1 > len2 else branches2[len1:]))
