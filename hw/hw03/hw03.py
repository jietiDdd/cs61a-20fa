...
def balanced(m):
    ...
    if balanced(left_arm_end) and balanced(right_arm_end):
        if total_weight(left_arm_end) * length(left_arm) != total_weight(right_arm_end) * length(right_arm):
            return False
        else:
            return True
    return False
...
def quadratic(x, a, b, c):
    ...
    """
    "*** YOUR CODE HERE ***"
    extreme_point, left_point, right_point = - b / (2 * a), lower_bound(x), upper_bound(x)
    def calculate(n):
        return a * n * n + b * n + c
    extreme, left_res, right_res = calculate(extreme_point), calculate(left_point), calculate(right_point)
    if left_point <= extreme_point <= right_point:
        return interval(min(extreme, left_res, right_res), max(extreme, left_res, right_res))
    else:
        return interval(min(left_res, right_res), max(left_res, right_res))
